...
如果不定义 `__reversed__`，`reversed()` 会回退到 `__len__` + `__getitem__`（效率低）。定义后直接返回高效的反向迭代器：
...
### `__bool__` —— 真值判断（`if obj:`）

默认情况下，所有 Python 对象都是 truthy。重写 `__bool__`（或 `__len__`）来定义对象何时为"假"：
...
> Python 先检查 `__bool__`，再检查 `__len__`（0 = falsy）。两者都没有则对象永远为 truthy。
...
- **`__getattribute__`**：每次属性访问都会被调用，优先级最高。重写时极度小心。
- **`__getattr__`**：仅在常规查找（包括 `__getattribute__`）失败后才调用。这是安全的重写选择。
...
## 10. 描述符协议：`__get__`、`__set__`、`__delete__`、`__set_name__`

描述符是 `@property`、`@classmethod`、`@staticmethod` 背后的机制。当类属性实现了 `__get__`，Python 就把它当作描述符：

```python
class PositiveNumber:
    def __set_name__(self, owner, name):
        self.name = name          # 属性在所属类上的名字
    ...
    # 没有 __delete__ → 删除属性会抛出 AttributeError

class Product:
    price = PositiveNumber()     # 描述符作为类属性
...
> `__set_name__` 在类创建时调用，告诉描述符自己的属性名——不需要手动传 "price" 两次。

## 11. 异步版本：`__aiter__`、`__anext__`、`__aenter__`、`__aexit__`

Python 为迭代器协议和上下文管理器协议提供了异步版本：
...
## 12. 类级钩子：`__init_subclass__`、`__class_getitem__`

### `__init_subclass__` —— 子类化时的钩子
...
### `__class_getitem__` —— 泛型类型标注

支持 `MyClass[int]` 语法。`list[int]`、`dict[str, int]` 就是靠它实现的：

```python
class Vector:
    def __class_getitem__(cls, params):
        print(f"Vector 参数化为 {params}")
        return cls

v_type = Vector[int]  # "Vector 参数化为 int"
```

> 实际开发中用 `typing.Generic` 即可，无需手动写 `__class_getitem__`。
...
| 真值判断 | `__bool__`、`__len__` | `bool(obj)`、`if obj:` |
...
| 描述符 | `__get__`、`__set__`、`__delete__`、`__set_name__` | `obj.attr`、`obj.attr = val`、`del obj.attr` |
| 异步 | `__aiter__`、`__anext__`、`__aenter__`、`__aexit__` | `async for`、`async with` |
| 类级钩子 | `__init_subclass__`、`__class_getitem__` | 子类创建、`Cls[int]` |
...
7. **描述符**（`__get__`/`__set__`）是 `@property` 的底层——用 `__set_name__` 让 API 更简洁。
8. 异步代码有自己的 dunder 方法——`__aiter__`、`__aenter__` 等。
